...
const MenuItems = () => {
  ...
  const openAdd = () => { setForm(emptyForm); setEditing(null); setImageFile(null); setModal(true); };
  const openEdit = (item) => {
  ...
  const handleDelete = async (id) => {
    ...
    try { await deleteMenuItem(id); toast.success('Deleted'); refetch(); }
  ...
  const filtered = filterCat ? items?.filter((i) => i.category?._id === filterCat || i.category === filterCat) : items;
  ...
  return (
    <div>
      <div className="flex flex-wrap justify-between items-center gap-3 mb-6">
        <h2 className="text-xl font-semibold">Menu Items ({filtered?.length || 0})</h2>
      ...
      {filtered?.length === 0 ? (
        <div className="card text-center py-12 text-gray-400"><p className="text-4xl mb-2">🍽️</p><p>No items yet.</p></div>
      ) : (
              ...
              {filtered?.map((item) => (
                <tr key={item._id} className="border-t hover:bg-gray-50">
                  <td className="px-4 py-3 flex items-center gap-3">
                    {item.image && <img src={imgUrl(item.image)} alt="" className="w-10 h-10 rounded-lg object-cover" onError={(e) => e.target.style.display='none'} />}
                    <div>
                  ...
                  <td className="px-4 py-3">
                    <span className={`text-xs px-2 py-0.5 rounded-full ${item.foodType === 'veg' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                      {item.foodType}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <button onClick={() => toggleAvail(item)} className={`text-xs px-2 py-0.5 rounded-full ${item.isAvailable ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                      {item.isAvailable ? 'Available' : 'Out of Stock'}
                    </button>
                  </td>
                  <td className="px-4 py-3 flex gap-2">
                    <button onClick={() => openEdit(item)} className="text-blue-500 hover:underline text-xs">Edit</button>
                    <button onClick={() => handleDelete(item._id)} className="text-red-400 hover:underline text-xs">Del</button>
                  </td>
                </tr>
              ))}
            </tbody>
